...
const CITIZENDIUM_BASE = 'https://citizendium.org';

interface CitizendiumResult {
  title: string;
  content: string;
  url: string;
  isFallback: boolean;
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  return json({
    title: topic,
    content: '',
    url: `${CITIZENDIUM_BASE}/wiki/${encodeURIComponent(topic)}`,
    isFallback: true,
    notFound: true,
    source: 'fallback'
  });
...
async function tryMediaWikiApi(topic: string): Promise<CitizendiumResult | null> {
  try {
    ...
    if (!response.ok) {
      console.log('Citizendium API returned:', response.status);
      return null;
    }
    ...
    if (content && content.length > 200) {
      return {
        ...
        content,
        url: page.fullurl || `${CITIZENDIUM_BASE}/wiki/${encodeURIComponent(topic)}`,
        isFallback: false,
        source: 'mediawiki'
      };
    }
  } catch (e) {
    console.log('Citizendium MediaWiki API error:', e instanceof Error ? e.message : e);
  }

  return null;
}

async function searchMediaWiki(topic: string): Promise<CitizendiumResult | null> {
  try {
    const params = new URLSearchParams({
      action: 'query',
      list: 'search',
    ...
    const articleResponse = await fetch(`${CITIZENDIUM_API}?${fetchParams}`, {
      headers: {
        'User-Agent': 'WikiArena/1.0 (Knowledge Comparison Platform)',
      },
    ...
    if (content && content.length > 200) {
      return {
        ...
        content,
        url: page.fullurl || `${CITIZENDIUM_BASE}/wiki/${encodeURIComponent(foundTitle)}`,
        isFallback: false,
        source: 'mediawiki'
      };
    }
  } catch (e) {
    console.log('Citizendium search error:', e instanceof Error ? e.message : e);
  }

  return null;
}

async function tryScrape(topic: string): Promise<CitizendiumResult | null> {
  try {
    const encodedTopic = encodeURIComponent(topic.replace(/\s+/g, '_'));
    const url = `${CITIZENDIUM_BASE}/wiki/${encodedTopic}`;
    ...
    // Check if it's a valid article (not a "page does not exist" page)
    ...
    console.log('Citizendium scrape error:', e instanceof Error ? e.message : e);
  }

  return null;
}

async function tryGrokApi(topic: string): Promise<CitizendiumResult | null> {
  if (!XAI_API_KEY) {
    return null;
  }
    ...
    const response = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are writing for Citizendium, an expert-guided encyclopedia that emphasizes accuracy and scholarly review. Write comprehensive, factual articles with academic rigor.

Format in Markdown with:
- Clear introduction defining the subject
- Multiple sections with ## headers
- Academic, neutral, authoritative tone
- Accurate facts and proper context
- Expert-level depth where appropriate
...
function convertHtmlToMarkdown(html: string, topic: string): string {
  ...
  content = content.replace(/citizendium/gi, 'this encyclopedia');
